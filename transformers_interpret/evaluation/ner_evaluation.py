...
BATCH_SIZE = 128 if CUDA_VISIBLE_DEVICES != 'cpu' else 32
        ...
        with alive_bar(total=len(self.ordered_attributions)) as bar:
            for document, doc_attributions in tqdm(zip(self.dataset, self.ordered_attributions)):
                if len(doc_attributions['entities']) > 0:
                    ...
                    assert len(first_entity['attribution_scores']) == len(document['input_ids']), f"{len(first_entity['attribution_scores'])} {len(document['input_ids'])} --- {document['document_id']} {first_entity['doc_id']} --- {first_entity['doc_title']}"
