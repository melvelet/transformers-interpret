        ...
        with alive_bar(total=len(self.ordered_attributions)) as bar:
            for document, doc_attributions in tqdm(zip(self.dataset, self.ordered_attributions)):
                if len(doc_attributions['entities']) > 0:
                    ...
                    assert len(first_entity['attribution_scores']) == len(document['input_ids']), f"{len(first_entity['attribution_scores'])} {len(document['input_ids'])} --- {document['document_id']} --- {doc_attributions['document_id']}"
